...
export function LoginModal({ isOpen }: LoginModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Main Card */}
        <div className="relative overflow-hidden rounded-2xl bg-[#0B1120]/95 border border-white/[0.07] shadow-2xl shadow-black/60 backdrop-blur-2xl">
          ...
          <div className="pt-10 pb-5 text-center relative">
            ...
            <div className="absolute top-6 left-1/2 -translate-x-1/2 w-32 h-32 rounded-full bg-[#D4A843]/6 blur-2xl pointer-events-none" />
            ...
            <div className="relative mb-5">
              <img
                ...
                className="h-22 w-22 mx-auto rounded-full shadow-lg shadow-[#D4A843]/15 border-2 border-[#D4A843]/25 object-cover"
                style={{ height: '5.5rem', width: '5.5rem' }}
              />
            </div>

            {/* République de Guinée */}
            <p className="text-[10px] text-[#D4A843] font-semibold tracking-[0.22em] uppercase">
              République de Guinée
            </p>

            {/* ARPT */}
            <h2 className="text-3xl font-bold text-slate-50 tracking-[0.15em] mt-3">
              ARPT
            </h2>

            {/* Full name */}
            <p className="text-[11px] text-slate-400 mt-1.5 max-w-[320px] mx-auto leading-relaxed">
              Autorité de Régulation des Postes et Télécommunications
            </p>

            {/* Separator */}
            <div className="mt-5 mx-auto w-48 h-px bg-gradient-to-r from-transparent via-[#D4A843]/25 to-transparent" />

            {/* Section title with lock */}
            <div className="mt-5 flex items-center justify-center gap-2.5">
              <div className="p-1.5 rounded-lg bg-[#D4A843]/10 border border-[#D4A843]/15">
          ...
          <form onSubmit={handleSubmit} className="px-8 pb-6 space-y-5">
            ...
            {/* Email */}
            <div>
              <label className="text-[10px] text-slate-400 mb-2 block font-semibold uppercase tracking-[0.1em]">
                Adresse email
              </label>
              <div className="relative group">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500 group-focus-within:text-[#D4A843]/60 transition-colors" />
                <input
                  type="email"
                  value={email}
            ...
            <div>
              ...
              <div className="relative group">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500 group-focus-within:text-[#D4A843]/60 transition-colors" />
                <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#D4A843] to-[#B8922E] text-sm font-bold text-[#080C1A] hover:brightness-110 active:brightness-95 transition-all flex items-center justify-center gap-2.5 disabled:opacity-50 disabled:hover:brightness-100 shadow-lg shadow-[#D4A843]/12 mt-2"
            >
              {loading ? (
                <>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {visibleAccounts.map((account) => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-left transition-all hover:bg-white/[0.05] border ${
                        email === account.email
                          ? 'border-[#D4A843]/25 bg-[#D4A843]/5'
          ...
          <div className="px-8 pb-6 text-center">
            {/* Subtle separator */}
            <div className="mx-auto w-24 h-px bg-gradient-to-r from-transparent via-white/[0.05] to-transparent mb-4" />

            <p className="text-[10px] text-slate-500 font-medium">
              Accès réservé au personnel autorisé de l&apos;ARPT
            </p>
            <p className="text-[9px] text-slate-600 mt-1.5 font-mono tracking-wide">

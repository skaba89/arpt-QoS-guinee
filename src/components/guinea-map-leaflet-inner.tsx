  ...
  useEffect(() => {
    ...
    if (useCNTDecoupage) {
      ...
      const getRegionMetric = (cntCode: string): { coverage: number; qos: number; color: string } => {
        ...
        // Find the parent region data and apply a small variation
        const cntRegion = cntRegions.find(r => r.code === cntCode);
        if (cntRegion) {
          const parentCode = Object.entries({
          ...
          }).find(([, v]) => v === cntRegion.ancienneRegion)?.[0];

          const parentData = regionDataByCode[parentCode || ''];
          if (parentData) {
            // Create varied data for new regions based on parent
            const seed = cntCode.charCodeAt(0) + cntCode.charCodeAt(cntCode.length - 1);
